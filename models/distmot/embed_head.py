"""
Head for generating final features and cal losses
Core codes
"""
...
from .memo_bank import MemoBank

@MODELS.register_module()
class DistEmbedHead(BaseModule):
    def __init__(self,
                 num_convs: int = 0,
                 num_fcs: int = 0,
                 roi_feat_size: int = 7,
                ...
                loss_track_hist: dict = dict(
                 ...
                 init_cfg: dict = dict(
                     ...
                     override=dict(
                         ...
                         std=0.01,
                         bias=0))):
        super(DistEmbedHead, self).__init__(init_cfg=init_cfg)
        ...
        # building mask module
        if mask_cfg is not None:
            self.mask_type = '1D' if '1D' in mask_cfg['type'] else '2D'
            self.mask_generator = MODELS.build(mask_cfg)
        else:
            self.mask_type = None
            self.mask_generator = None

        # building memory bank
        self.memo_bank = MemoBank(memo_cfg)  # key: instances_id, value: dict
        if loss_track_hist is not None:
            self.loss_track_hist = MODELS.build(loss_track_hist)
        else:
            self.loss_track_hist = None
    ...
    def loss(self, key_roi_feats: Tensor, ref_roi_feats: Tensor,
        ...
        losses = self.loss_by_feat(key_track_feats, ref_track_feats,
                                   ...
                                   gt_match_indices_list, video_id_list)
        return losses
    ...
    def loss_by_feat(self, key_track_feats: Tensor, ref_track_feats: Tensor,
        ...
        assert len(hist_dists) == len(dists)
        ...
        loss_hist = torch.tensor([0.]).to(loss_track.device)
        """Margin L2 Loss
        for _dists, _labels in zip(hist_dists, hist_labels):            
        ...
        # losses['loss_history'] = self.memo_bank.cal_history_loss()
