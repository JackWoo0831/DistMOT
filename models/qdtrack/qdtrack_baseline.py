...
class QDTrack_baseline(BaseMOTModel):
    ...
    def predict(self,
        ...
        for frame_id in range(video_len):
            ...
            # NOTE: For debug: visualize region proposals
            # frame_pred_track_instances = det_results[0]
            # frame_pred_track_instances.instances_id = torch.zeros((frame_pred_track_instances.bboxes.shape[0], )).int()
    ...
    def loss(self, inputs: Tensor, data_samples: TrackSampleList,
        ...
        x = self.detector.extract_feat(key_imgs)  # four scales, (H/4, W/4) -> (H/32, W/32)
        ...
        rpn_losses, rpn_results_list = self.detector.rpn_head. \
            loss_and_predict(x,
                             ...
                             **kwargs)  # rpn_results_list: List[InstanceData]
