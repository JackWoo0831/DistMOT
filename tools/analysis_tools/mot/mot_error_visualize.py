...
ANNO_PREFIX = osp.join(DATA_ROOT, 'VisDrone2019-MOT-val', 'annotations')
...
def parse_args():
    ...
    parser.add_argument(
        '--fps', type=int, default=3, help='FPS of the output_dir video')
    parser.add_argument(
        '--backend',
        type=str,
        choices=['cv2', 'plt'],
...
def compare_res_gts(results_dir: str, dataset: Dataset, video_name: str):
    ...
    gt_file = osp.join(ANNO_PREFIX, f'{video_name}.txt')
...
def main():
    ...
    assert args.show or args.output_dir, \
    ...
    for i in range(len(dataset)):
        ...
        for data_info in video_info['images']:
            ...
            video_name = split_path[-2]  # NOTE here
    ...
    video_names = ['uav0000086_00000_v']  # NOTE
    ...
    for video_name in video_names:
        ...
        print_log(f'Done! Visualization images are saved in '
                  f'\'{args.output_dir}/{video_name}\'')

        mmcv.frames2video(
            f'{args.output_dir}/{video_name}',
            f'{args.output_dir}/{video_name}.mp4',
            fps=args.fps,
            fourcc='mp4v',
        ...
        print_log(
            ...
            f'\'{args.output_dir}/{video_name}.mp4\' with a FPS of {args.fps}')


if __name__ == '__main__':
    main()

    # python tools/analysis_tools/mot/mot_error_visualize.py configs/distmot/distmot_yolox_tiny_visdrone.py --result-dir txt_results --output-dir qdtrack_baseline_vis